// NodeMBTCP - A library for communication using Modbus/TCP from node.js. 

var net = require("net");
var _ = require("underscore");
var util = require("util");
var effectiveDebugLevel = 2; // intentionally global, shared between connections
var wordSwapFloat = true; // TODO - move into prototype
var wordSwapInt32 = false; // TODO - move into prototype
...
function NodeMBTCP(){
	...
	self.wordSwapFloat = false; // Not used yet
	self.wordSwapInt32 = false; // Not used yet
...
NodeMBTCP.prototype.initiateConnection = function (cParam, callback) {
	var self = this;
	if (cParam === undefined) { cParam = {port: 502, host: '192.168.8.106'}; }
	...
	if (typeof(cParam.path) !== 'undefined') {
		Connection_Path = new Buffer(path);
	}
...
NodeMBTCP.prototype.dropConnection = function () {
	var self = this;
...
NodeMBTCP.prototype.setPath = function (path) {
	Connection_Path = new Buffer(path);
}
...
NodeMBTCP.prototype.sendReadPacket = function() {
	...
	for (i = 0;i < self.readPacketArray.length; i++) {
		...
		// This is the overall message length for the Modbus request, minus most of the header
...
//		setTimeout(connectNow, 2000, connectionParams);
	...
	if (data.length < (data.readInt16BE(4) + 6)) {
...
NodeMBTCP.prototype.readResponse = function(data, foundSeqNum) {
	...
	for (itemCount = 0; itemCount < self.readPacketArray[foundSeqNum].itemList.length; itemCount++) {
		dataPointer = processMBPacket(data, self.readPacketArray[foundSeqNum].itemList[itemCount], dataPointer);
...
//				outputLog(self.globalReadBlockList[i].itemReference[k].byteBuffer);
				processMBReadItem(self.globalReadBlockList[i].itemReference[k]);
...
//		outputLog("Processing an undefined packet, likely due to timeout error");
	...
	if (remainingLength < 9) {
		theItem.valid = false;
		if (typeof(theData) !== "undefined") {
			theItem.errCode = 'Malformed Modbus Packet - Less Than 9 Bytes.  TDL' + theData.length + 'TP' + thePointer + 'RL' + remainingLength;
	...
	if (theData[2] !== 0x00 || theData[3] !== 0x00) {
		theItem.valid = false;
	...
	if (theData.length - 6 !== theData.readUInt16BE(4)) {  // Already checked before getting here but check again to be sure.  
		theItem.valid = false;
...
function processMBReadItem(theItem) {
...
//					theItem.value.push(theItem.byteBuffer.readFloatBE(thePointer));
					theItem.value.push(getFloat(theItem.byteBuffer,thePointer));
					break;
				case "DWORD":
//					theItem.value.push(theItem.byteBuffer.readUInt32BE(thePointer));
					theItem.value.push(getUInt32(theItem.byteBuffer,thePointer));
					break;
				case "DINT":
//					theItem.value.push(theItem.byteBuffer.readInt32BE(thePointer));
					theItem.value.push(getInt32(theItem.byteBuffer,thePointer));
					break;
				case "INT":
...
//				theItem.value = theItem.byteBuffer.readFloatBE(thePointer);
				theItem.value = getFloat(theItem.byteBuffer,thePointer);
				break;
			case "DWORD":
//				theItem.value = theItem.byteBuffer.readUInt32BE(thePointer);
				theItem.value = getUInt32(theItem.byteBuffer,thePointer);
				break;
			case "DINT":
//				theItem.value = theItem.byteBuffer.readInt32BE(thePointer);
				theItem.value = getInt32(theItem.byteBuffer,thePointer);
				break;
			case "INT":
...
function bufferizeMBItem(theItem) {	
	...
	if (theItem.arrayLength > 1) {
		...
		for (arrayIndex = 0; arrayIndex < theItem.arrayLength; arrayIndex++) {
			switch(theItem.datatype) {
				case "REAL":
					setFloat(theItem.writeBuffer, thePointer, theItem.writeValue[arrayIndex]);
//					theItem.writeBuffer.writeFloatBE(theItem.writeValue[arrayIndex], thePointer);
					break;
				case "DWORD":
					setUInt32(theItem.writeBuffer, thePointer, theItem.writeValue[arrayIndex]);
//					theItem.writeBuffer.writeInt32BE(theItem.writeValue[arrayIndex], thePointer);
					break;
				case "DINT":
					setInt32(theItem.writeBuffer, thePointer, theItem.writeValue[arrayIndex]);
//					theItem.writeBuffer.writeInt32BE(theItem.writeValue[arrayIndex], thePointer);
					break;
				case "INT":
...
//??					theItem.writeBuffer.writeUInt8(theItem.writeValue.toCharCode(), thePointer);
		...
		switch(theItem.datatype) {
			...
			case "REAL":
				setFloat(theItem.writeBuffer, thePointer, theItem.writeValue);
//				theItem.writeBuffer.writeFloatBE(theItem.writeValue, thePointer);
				break;
			case "DWORD":
				setUInt32(theItem.writeBuffer, thePointer, theItem.writeValue);
//				theItem.writeBuffer.writeUInt32BE(theItem.writeValue, thePointer);
				break;
			case "DINT":
				setInt32(theItem.writeBuffer, thePointer, theItem.writeValue);
...
function MBAddrToBuffer(addrinfo, isWriting) { // SLCAddrToBufferA2
	...
	if (addrinfo.subtractor == 30001 || addrinfo.subtractor == 40001) {
		MBCommand.writeUInt16BE(addrinfo.offset, 2);
		MBCommand.writeUInt16BE(addrinfo.byteLength/2, 4);
	}

	if (addrinfo.subtractor == 1 || addrinfo.subtractor == 10001) {
		MBCommand.writeUInt16BE(addrinfo.offset, 2);
...
//	outputLog('StartRegisterString[0] is ' + startRegisterString[0],2);
	...
	if (theItem.startRegister <= 0 || theItem.startRegister == 10000 || (theItem.startRegister > 19999 && theItem.startRegister < 30001) || theItem.startRegister == 40000 || theItem.startRegister > 49999) {
		outputLog("Error - Starting register  must be 1-9999, 10001-19999, 30001-39999, 40001-49999");
		return undefined;
	}
	...
	if (theItem.subtractor !== 30001 && theItem.subtractor !== 40001 && typeof(theItem.bitOffset) !== 'undefined') {
...
function setUInt32(buf, ptr, val) {
	var newBuf = new Buffer(4);
	if (wordSwapInt32) {
